...
#ifndef LOUVAIN_BLIM_THREAD
/** Number of threads per block for a thread-per-vertex kernel. */
#define LOUVAIN_BLIM_THREAD 32
/** Number of threads per block for a block-per-vertex kernel. */
#define LOUVAIN_BLIM_BLOCK  128
#endif
#pragma endregion
// NOTE: Use GRID_LIMIT_MAP_CUDA instead of GRID_LIMIT_CUDA?
 ...
 */
template <int SDEG=128, class O, class K, class V, class W>
...
inline void louvainVertexWeightsThreadCuW(W *vtot, const O *xoff, const K *xdeg, const V *xwei, K NB, K NE) {
  const int B = blockSizeCu(NE-NB, LOUVAIN_BLIM_THREAD);
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_CUDA);
...
void __global__ louvainVertexWeightsBlockCukW(W *vtot, const O *xoff, const K *xdeg, const V *xwei, K NB, K NE) {
  DEFINE_CUDA(t, b, B, G);
  __shared__ W cache[LOUVAIN_BLIM_BLOCK];
...
inline void louvainVertexWeightsBlockCuW(W *vtot, const O *xoff, const K *xdeg, const V *xwei, K NB, K NE) {
  const int B = blockSizeCu<true>(NE-NB, LOUVAIN_BLIM_BLOCK);
  const int G = gridSizeCu <true>(NE-NB, B, GRID_LIMIT_CUDA);
  louvainVertexWeightsBlockCukW<SDEG><<<G, B>>>(vtot, xoff, xdeg, xwei, NB, NE);
}


/**
 * Find the total edge weight of each community, using thread-per-vertex approach [kernel].
...
/**
 * Find the total edge weight of each community, using thread-per-vertex approach.
 * @param ctot total edge weight of each community (updated, must be initialized)
 * @param vcom community each vertex belongs to
 * @param vtot total edge weight of each vertex
 * @param NB begin vertex (inclusive)
...
/**
 * Initialize communities such that each vertex is its own community, using thread-per-vertex approach [kernel].
...
/**
 * Initialize communities such that each vertex is its own community, using thread-per-vertex approach.
 * @param vcom community each vertex belongs to (output)
 * @param ctot total edge weight of each community (output)
 * @param vtot total edge weight of each vertex
 * @param NB begin vertex (inclusive)
 ...
 */
template <int HTYPE=3, int BLIM=LOUVAIN_BLIM_THREAD, class O, class K, class V, class W, class F>
void __global__ louvainMoveThreadCukU(double *el, K *vcom, W *ctot, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const W *vtot, W M, W R, K NB, K NE, bool PICKLESS) {
  ...
  for (K u=NB+B*b+t; u<NE; u+=G*B) {
    ...
    size_t EO = xoff[u];
    size_t EN = xdeg[u];
    if (EN==0 || EN>=BLIM) continue;  // Skip isolated and high-degree vertices
    ...
    W vdout = hashtableGetCud(hk, hv, H, T, d+1);  // NOTE: Can be optimized
 ...
 * @param PICKLESS allow only picking smaller community id?
 */
template <int HTYPE=3, int BLIM=LOUVAIN_BLIM_BLOCK, class O, class K, class V, class W, class F>
void __global__ louvainMoveBlockCukU(double *el, K *vcom, W *ctot, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const W *vtot, W M, W R, K NB, K NE, bool PICKLESS) {
  ...
  for (K u=NB+b; u<NE; u+=G) {
    ...
    size_t EO = xoff[u];
    size_t EN = xdeg[u];
    if (EN==0 || EN < BLIM) continue;  // Skip isolated and low-degree vertices
    size_t H = nextPow2Cud(EN) - 1;
    size_t T = nextPow2Cud(H)  - 1;
...
inline int louvainMoveCuU(double *el, K *vcom, W *ctot, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const W *vtot, K N, K NL, W M, W R, int L, FC fc) {
  int l = 0;
  double elH = 0;
  const int PICKSTEP = 4;
  while (l<L) {
    bool PICKLESS = l % PICKSTEP == 0;  // (l + PICKSTEP / 2) % PICKSTEP == 0;
    ...
    TRY_CUDA( cudaMemcpy(&elH, el, sizeof(double), cudaMemcpyDeviceToHost) ); ++l;
    if (!PICKLESS && fc(elH, l)) break;
  }
  return l > 1 || elH ? l : 0;
}
#pragma endregion
...
/**
 * Examine if each community exists, using thread-per-vertex approach [kernel].
...
/**
 * Examine if each community exists, using thread-per-vertex approach.
 * @param C number of communities (updated, must be initialized)
 * @param a does each community exist (updated, must be initialized)
 * @param vcom community each vertex belongs to
 * @param NB begin vertex (inclusive)
...
/**
 * Find the total degree of each community, using thread-per-vertex approach [kernel].
...
/**
 * Find the total degree of each community, using thread-per-vertex approach.
 * @param a total degree of each community (updated, must be initialized)
...
/**
 * Find the number of vertices in each community, using thread-per-vertex approach [kernel].
...
/**
 * Find the number of vertices in each community, using thread-per-vertex approach.
 * @param a number of vertices belonging to each community (updated, must be initialized)
 * @param vcom community each vertex belongs to
 * @param NB begin vertex (inclusive)
 * @param NE end vertex (exclusive)
...
/**
 * Populate community vertices into a CSR structure, using thread-per-vertex approach [kernel].
...
/**
 * Populate community vertices into a CSR structure, using thread-per-vertex approach.
 * @param cdeg number of vertices in each community (updated)
 ...
 */
template <class K>
inline void louvainCommunityVerticesCuW(K *coff, K *cdeg, K *cedg, K *bufk, const K *vcom, K N, K C) {
  size_t B = max(N+1, K(BLOCK_LIMIT_CUDA));
...
/**
 * Update community membership in a tree-like fashion (to handle aggregation), using thread-per-vertex approach [kernel].
...
/**
 * Update community membership in a tree-like fashion (to handle aggregation), using thread-per-vertex approach.
 * @param a output community each vertex belongs to (updated)
 * @param vcom community each vertex belongs to (at this aggregation level)
 * @param NB begin vertex (inclusive)
 * @param NE end vertex (exclusive)
 ...
 */
template <class K>
inline void louvainRenumberCommunitiesCuU(K *vcom, K *cext, K *bufk, K N) {
  size_t B = max(N+1, K(BLOCK_LIMIT_CUDA));
 ...
 */
template <int HTYPE=3, int BLIM=LOUVAIN_BLIM_THREAD, class O, class K, class V, class W>
void __global__ louvainAggregateEdgesThreadCukU(K *ydeg, K *yedg, V *ywei, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const K *vcom, const O *coff, const K *cedg, const O *yoff, K CB, K CE) {
  ...
  for (K c=CB+B*b+t; c<CE; c+=G*B) {
    ...
    if (CN==0 || EN >= BLIM) continue;  // Skip empty communities, and communities with high total degree
    size_t H = nextPow2Cud(EN) - 1;
    size_t T = nextPow2Cud(H)  - 1;
    K *hk = bufk + 2*EO;
    W *hv = bufw + 2*EO;
...
inline void louvainAggregateEdgesThreadCuU(K *ydeg, K *yedg, V *ywei, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const K *vcom, const O *coff, const K *cedg, const O *yoff, K CB, K CE) {
  ...
  const int G = gridSizeCu (CE-CB, B, GRID_LIMIT_CUDA);
 ...
 * @param CE end community (exclusive)
 */
template <int HTYPE=3, int BLIM=LOUVAIN_BLIM_BLOCK, class O, class K, class V, class W>
void __global__ louvainAggregateEdgesBlockCukU(K *ydeg, K *yedg, V *ywei, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const K *vcom, const O *coff, const K *cedg, const O *yoff, K CB, K CE) {
  ...
  for (K c=CB+b; c<CE; c+=G) {
    ...
    size_t CO = coff[c];
    size_t CN = coff[c+1] - coff[c];
    if (CN==0 || EN < BLIM) continue;  // Skip empty communities and communities with small total degree
    size_t H = nextPow2Cud(EN) - 1;
    size_t T = nextPow2Cud(H)  - 1;
    K *hk = bufk + 2*EO;
    W *hv = bufw + 2*EO;
...
inline void louvainAggregateEdgesBlockCuU(K *ydeg, K *yedg, V *ywei, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const K *vcom, const O *coff, const K *cedg, const O *yoff, K CB, K CE) {
  const int B = blockSizeCu<true>(CE-CB, BLIM);  // Why 4*BLIM?
  const int G = gridSizeCu <true>(CE-CB, B, GRID_LIMIT_CUDA);
...
inline void louvainAggregateCuW(O *yoff, K *ydeg, K *yedg, V *ywei, O *bufo, K *bufk, W *bufw, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, const K *vcom, const O *coff, const K *cedg, K N, K C) {
  size_t B = max(N+1, K(BLOCK_LIMIT_CUDA));
 ...
 * @param fi initializing community membership and total vertex/community weights (vcom, vtot, ctot, xoff, xdeg, xedg, xwei, NL)
 * @param fm marking affected vertices (vaff, xoff, xdeg, xedg, xwei, NL)
...
inline auto louvainInvokeCuda(const G& x, const LouvainOptions& o, FI fi, FM fm) {
  ...
  int    P = o.maxPasses, p = 0;      // 5
  // Allocate buffers on host, original and compressed.
  vector<K> ucom(S), ucomc(N);  // Community membership (first pass)
  vector<W> utot;       // Total vertex weights (first pass)
  vector<W> ctot;       // Total community weights (first pass)
  ...
  K *ucomD = nullptr, *vcomD = nullptr;  // Community membership (first and subsequent passes)
  W *vtotD = nullptr;  // Total vertex weights
  W *ctotD = nullptr;  // Total community weights
  O *bufoD = nullptr;  // Buffer for exclusive scan
  K *bufkD = nullptr;  // Keys of hashtable
  W *bufwD = nullptr;  // Values of hashtable
  O *xoffD = nullptr, *yoffD = nullptr;  // Offsets of input and aggregated graphs
  K *xdegD = nullptr, *ydegD = nullptr;  // Degrees of input and aggregated graphs
  K *xedgD = nullptr, *yedgD = nullptr;  // Edge keys of input and aggregated graphs
  V *xweiD = nullptr, *yweiD = nullptr;  // Edge values of input and aggregated graphs
  ...
  float  t = measureDurationMarked([&](auto mark) {
    ...
    ti += mark([&]() { fi(ucomD, vtotD, ctotD, xoffD, xdegD, xedgD, xweiD, N, NL); });
    // Mark affected vertices.
    tm += mark([&]() { fm(vaffD, xoffD, xdegD, xedgD, xweiD, N, NL); });
    ...
    mark([&]() {
      ...
      for (l=0, p=0; M>0 && P>0;) {
        ...
        int  m = 0;
        tl += measureDuration([&]() {
          if (isFirst) m = louvainMoveCuU<HTYPE>(elD, ucomD, ctotD, vaffD, bufkD, bufwD, xoffD, xdegD, xedgD, xweiD, vtotD, K(N),  K(NL), M, R, L, fc);
          else         m = louvainMoveCuU<HTYPE>(elD, vcomD, ctotD, vaffD, bufkD, bufwD, xoffD, xdegD, xedgD, xweiD, vtotD, K(GN), K(GN), M, R, L, fc);
        });
        l += max(m, 1); ++p;
        if (m<=1 || p>=P) break;
        uint64_cu CN = 0;
        ...
        if (double(CN)/GN >= o.aggregationTolerance) break;  // o.aggregationTolerance = min(max(1 - 0.025 * X / N, 0.1), 0.9)
        if (isFirst) louvainRenumberCommunitiesCuU(ucomD, cdegD, bufkD, K(N));
        else         louvainRenumberCommunitiesCuU(vcomD, cdegD, bufkD, K(GN));
        if (isFirst) {}
        else         louvainLookupCommunitiesCuU(ucomD, vcomD, K(), K(N));
        ta += measureDuration([&]() {
          if (isFirst) louvainCommunityVerticesCuW(coffD, cdegD, cedgD, bufkD, ucomD, K(N),  K(CN));
          else         louvainCommunityVerticesCuW(coffD, cdegD, cedgD, bufkD, vcomD, K(GN), K(CN));
          if (isFirst) louvainAggregateCuW(yoffD, ydegD, yedgD, yweiD, bufoD, bufkD, bufwD, xoffD, xdegD, xedgD, xweiD, ucomD, coffD, cedgD, K(N),  K(CN));
          else         louvainAggregateCuW(yoffD, ydegD, yedgD, yweiD, bufoD, bufkD, bufwD, xoffD, xdegD, xedgD, xweiD, vcomD, coffD, cedgD, K(GN), K(CN));
        });
...
inline auto louvainStaticCuda(const G& x, const LouvainOptions& o={}) {
  ...
  using  W = LOUVAIN_WEIGHT_TYPE;
  using  F = char;
  auto  fi = [](K *vcomD, W *vtotD, W *ctotD, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, K N, K NL) {
    louvainVertexWeightsThreadCuW(vtotD, xoff, xdeg, xwei, K(), NL);
    louvainVertexWeightsBlockCuW (vtotD, xoff, xdeg, xwei, NL,  N);
    louvainInitializeCuW(vcomD, ctotD, vtotD, K(), N);
  };
  auto  fm = [](F *vaffD, const O *xoff, const K *xdeg, const K *xedg, const V *xwei, K N, K NL) {
    fillValueCuW(vaffD, N, F(1));
  };
