...
inline void runBatches(const G &x, R &rnd, F fn)
{
  ...
  for (int epoch = 0;; ++epoch)
  {
    ...
    if (d >= BATCH_DELETIONS_END && i >= BATCH_INSERTIONS_END)
      break;
...
{
  if (NUM_THREADS_MODE == "with-batch")
    runThreadsWithBatch(epoch, fn);
  else
    runThreadsAll(fn);
}
#pragma endregion
...
void runExperiment(const G &x)
{
  ...
  runBatches(x, rnd, [&](const auto &y, auto deletionsf, const auto &deletions, auto insertionsf, const auto &insertions, int sequence, int epoch)
    ...
    runThreads(epoch, [&](int numThreads) {
      ...
      auto c1 = louvainStaticCuda(y, {repeat});
      flog(c1, "louvainStaticCuda");
